...
export default function BuyerHome(){
    ...
    // useEffect(()=>{
    //     fetch("http://localhost:8080/getbuyerbyloginid/"+JSON.parse(localStorage.getItem("loggedUser")).id)
    //     .then(res => res.json())
    //     .then(data => {setBuyer(data);localStorage.setItem("loggedBuyer",JSON.stringify(data));})
    //    // return()=>{cont.abort()};
    //  },[]);
    ...
    const [id, setId] = useState([]);
            ...
            if (items) {
            setId(items);
            fetch("http://localhost:8080/getbuyerbyloginid/"+items)
            .then(res => res.json())
            .then(data => {setBuyer(data);localStorage.setItem("loggedBuyer",JSON.stringify(data))})
            }
            }, []);
    ...
    const getReqInfo=()=>{
        ...
        const reqOptions = {
            ...
            body: JSON.stringify({
                ...
                buyer_id: JSON.parse(localStorage.getItem('loggedBuyer')).id,
                property_id: JSON.parse(localStorage.getItem('property')).id,
                lname:JSON.parse(localStorage.getItem('loggedBuyer')).lname,
                fname:JSON.parse(localStorage.getItem('loggedBuyer')).fname,
                email:JSON.parse(localStorage.getItem('loggedBuyer')).login_id?.email,
                contact_no:JSON.parse(localStorage.getItem('loggedBuyer')).contact_no
